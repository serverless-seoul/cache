...
const keys = Array.from(new Array(100)).map((_, i ) => `get_${i}`);
...
if (isMainThread) {
  console.log("Benchmarking: Get single item (%d iterations / %d keys)", iterations, keys.length); // tslint:disable-line
}
