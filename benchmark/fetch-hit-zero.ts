...
const createSuite = (branch: Branch, node: Node, version: Version, pipelining: Pipelining): [string, Test] => {
  ...
  return [
    ...
    ].join(" | "),
    {
      ...
      test() {
        return (fetcher.fetch as any)(`fetcher-zero-${Math.floor(Date.now() * Math.random())}`, 3600, async () => ({
          cachedAt: Date.now(),
        }));
      },
    },
...
if (isMainThread) {
  console.log("Benchmarking: Fetch single item (%d iterations, 0% hit ratio)", iterations); // tslint:disable-line
}
