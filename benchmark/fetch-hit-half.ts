...
const cachedKey = "fetch-half-hit";
  ...
  return [
    ...
    ].join(" | "),
    {
      async before() {
        ...
        await (fetcher.fetch as any)(cachedKey, 3600, async () => ({
          cachedAt: Date.now(),
        }));
      },
      async after() {
      ...
      test() {
        const key = Math.random() < 0.5 ? cachedKey : `fetcher-half-${Math.floor(Date.now() * Math.random())}`;
...
if (isMainThread) {
  console.log("Benchmarking: Fetch single item (%d iterations, 50% hit ratio)", iterations); // tslint:disable-line
}
