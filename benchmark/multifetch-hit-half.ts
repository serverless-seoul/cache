...
const createSuite = (node: Node, version: Version, pipelining: Pipelining): [string, Test] => {
  ...
  const cachedArgs = args.slice(0, Math.floor(args.length / 2));
...
if (isMainThread) {
  console.log("Benchmarking: Fetch multiple items (develop only, %d iterations, %d keys, 50% hit ratio)", iterations, batchSize); // tslint:disable-line
}
